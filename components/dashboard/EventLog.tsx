...
const severityLabels: Record<ClusterEvent["severity"], string> = {
  info: "INFO",
  success: "ÉXITO",
  warning: "ADVERTENCIA",
  error: "ERROR",
...
export function EventLog({ events }: { events: ClusterEvent[] }) {
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-950/80 p-4">
      <h2 className="text-lg font-semibold text-zinc-100">Registro de Eventos</h2>
        ...
        <ul className="space-y-2">
          {events.map((event) => (
            <li key={event.id} className="border-b border-zinc-900 pb-2 last:border-b-0">
              ...
              <span className={severityStyle[event.severity]}>{severityLabels[event.severity]}</span>{" "}
              <span className="text-zinc-300">{event.eventType}</span>{" "}
              {event.nodeId ? <span className="text-blue-300">nodo={event.nodeId}</span> : null}{" "}
